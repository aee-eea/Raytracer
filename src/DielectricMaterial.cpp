...
DielectricMaterial::DielectricMaterial(float refractionIndex) : refractionIndex{refractionIndex}{}
...
bool DielectricMaterial::scatter(const Ray& ray, const HitRecord& rec, glm::dvec3& attenuation, Ray& scattered,const Raytracer& env) const{
    ...
    float cosTheta = std::fmin(glm::dot(-glm::normalize(ray.dir),rec.normal),1.0);
